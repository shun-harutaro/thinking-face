...
while True:
    ...
    # 分類器を用いて顔検出を行う
    facerect = cascade.detectMultiScale(gray,scaleFactor=1.1,minNeighbors=1,minSize=(10,10))#※分類器による顔検出
    if len(facerect) > 0:
        # 検出した顔を加工（四角枠の描画もしくはモザイク化）
        for rect in facerect:
            out = imgTF.resize((rect[2],rect[3]))
            mask = out.split()[3]
            img.paste(out,(rect[0],rect[1]),mask)
